...
namespace Bot_Dofus_1._29._1.Scripts.Manejadores
{
    public class ManejadorAcciones : IDisposable
    {
        ...
        private async void evento_Movimiento_Celda(bool es_correcto)
        {
            ...
            if (accion_actual is PeleasAccion)
            {
                if (es_correcto)
                {
                    for (int delay = 0; delay < 10000 && cuenta.accountState != AccountState.FIGHTING; delay += 500)
        ...
        private void get_Recoleccion_Acabada(GatheringResult resultado)
        {
            if (!cuenta.script.corriendo)
                return;
