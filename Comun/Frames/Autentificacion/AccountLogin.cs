...
namespace Bot_Dofus_1._29._1.Comun.Frames.LoginCuenta
{
    public class AccountLogin : Frame
    {
        ...
        [PaqueteAtributo("Af")]
        public void GetLoginQueue(TcpClient prmClient, string prmPacket)
        {
            var queueData = prmPacket.Substring(2).Split('|');
            prmClient.account.Logger.LogInfo("Connexion", $"Position dans la file d'attente : {queueData[0]}/{queueData[1]}");
        }
